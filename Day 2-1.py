...
def validate ( passwords ):
    ...
    for p in passwords:
        ...
        policyparts = _[0].split(' ')
        policyparts_q = policyparts[0].split('-')
        policyparts_q_min = int(policyparts_q[0])
        policyparts_q_max = int(policyparts_q[1])

        policyparts_letter = policyparts[1]

        passw = _[1]
        occurance = passw.count(policyparts_letter)
        if  occurance >= policyparts_q_min and occurance <= policyparts_q_max: valid+= 1
